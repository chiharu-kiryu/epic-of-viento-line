...
默认只输出计划。`--write` 保留全部文档 ID、原文、素材绑定和路径，仅更新文档描述。写入全过程持有登记锁：先在作品 `.viento/migrations/` 记录修改前后描述，再原子替换每份描述文件。重复执行不产生新身份或重复关系。

已有关系默认保持原样。需要补充或纠正归属时，可以再次传入 `--shared-owners`：映射中明确列出的文档按所列目标更新 `part-of`，其他关系保持原样，仍被保留的归属目标沿用原有名称和备注。先不加 `--write` 检查计划，再确认写入；无效或循环归属会被拒绝。每次写入使用独立迁移日志，不覆盖较早的恢复记录；作品内部的链接目录或链接锁文件不能作为迁移写入位置。

...
网络从代码静态梳理，不是运行时追踪。本轮使用桌面内置 Node v24.20.0 和真实原生归档程序，应用检查 586 项全部通过、零失败、零跳过；完整原生库 22 通过、1 忽略，glib 回归 5 项通过。[本轮验证记录](test-results/native-current-2026-09-24/results.json)保存三组实际 Linux 窗口流程、音视频播放和功能图交互结果。每条链路的验证入口不代表所有状态均已覆盖；测试仅使用隔离的临时作品，日常正文、素材和备份未读写。
...
修订 64 保留 170 份代码指纹、181 处节点源码引用、499 条本地导入及 388 个测试声明，应用代码未改变。四十六轮验证和历史发布证据保留，交互页内嵌数据与 JSON 一致。本轮重新构建当前 Linux Tauri，完成旧 v2 编辑、通用 v3 项目和作品库三组窗口流程，并验证七种音频、两种视频的解码、播放、暂停与跳转。功能图四视图及导航实测已完成。详见[当前桌面实测报告](NATIVE_WORKFLOW_TEST_b.4.3.md)；真实大体积迁移、Windows/macOS、实体扬声器和外置设备断连/重连仍未验证。

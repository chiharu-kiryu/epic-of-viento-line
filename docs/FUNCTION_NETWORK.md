...
此页枚举 **2026-09-13、b.2.8.2 发布源码**的实际功能入口、业务步骤、接口和数据落点，包含音频支持、类型取消/返回及[按图排查的十轮修复](NETWORK_BUGFIX_b.2.8.1.md)。此为修订 12，业务链路与修订 11 一致，JSON 另记本次升版的清单、锁文件、页面版本指纹及一处 Rust 格式整理。基线提交为 `51d522cb919c20b15815b129bd62e859af9a97a0`；逐文件 SHA-256 保存在 JSON 快照中。
...
网络从代码静态梳理，不是运行时追踪。随后按图排查并修复问题，本轮启用原生归档互通的 app-only 检查通过 193 项测试，在此前 [编辑器](NATIVE_WORKFLOW_TEST_b.2.8.1.md) 与 [作品库](NATIVE_LIBRARY_TEST_b.2.8.1.md) 实测上补齐了 [编辑器导出、原生保存与过期重试](NATIVE_EXPORT_TEST_b.2.8.1.md)；每条链路附的“已有验证入口”仍不代表该链路所有运行状态均已覆盖。测试使用临时作品，没有修改日常作品。业务枚举按职责归并，并不声称覆盖所有运行时状态组合。
...
读取文件原文与版本，建立草稿基线。 使用源文件而非展示索引还原正文；源读取失败不会把展示内容当成权威稿。

已有验证入口：[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)、[scripts/tests/metadata-conversion.test.mjs](../scripts/tests/metadata-conversion.test.mjs)。
...
优先加载项目模板并建议安全的文件名、目录和扩展名；界面与服务共用新建路径校验。 模板读取失败明确提示空模板；重试成功恢复正常提示。选定类型不会因为更换保存目录而丢失。

已有验证入口：[scripts/tests/templates.test.mjs](../scripts/tests/templates.test.mjs)、[scripts/tests/generic-project.test.mjs](../scripts/tests/generic-project.test.mjs)、[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)。
...
区块来自当前源码草稿，序列化时只替换改动片段。 不是把解析后的字段对象整体重写为源文；JSON/YAML 等不适用的模式受 UI 限制。

已有验证入口：[scripts/tests/editor-draft.test.mjs](../scripts/tests/editor-draft.test.mjs)、[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)。
...
校验 expectedVersion，原子保存原文，失效服务缓存，再重建和重新载入。 源文件保存和重建是两个阶段；重建或索引读取失败时明确报错，已保存正文继续可编辑并允许重试。

已有验证入口：[scripts/tests/doc-api.test.mjs](../scripts/tests/doc-api.test.mjs)、[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)、[scripts/tests/call-paths.test.mjs](../scripts/tests/call-paths.test.mjs)。
...
在登记锁内重新读取类型、检查可迁移位置冲突，再建立 UUID 并独占创建源文件。 保护大小写/Unicode 等价名称、父目录和缺失源文件的登记身份；失败回滚本次登记，保存成功后即作为已有文档继续编辑。

已有验证入口：[scripts/tests/generic-project.test.mjs](../scripts/tests/generic-project.test.mjs)、[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)。
...
选择读取最新、保留草稿、明确强制覆盖或取消。 强制覆盖必须由冲突 UI 明确选择；默认不会静默压过另一版本。
...
检测内容及新建路径差异，保留或明确丢弃草稿。 浏览器 beforeunload 和桌面关闭链路也查看脏状态；不提供磁盘自动草稿恢复。

已有验证入口：[scripts/tests/editor-runtime.test.mjs](../scripts/tests/editor-runtime.test.mjs)、[desktop/tests/native-smoke.py](../desktop/tests/native-smoke.py)。
...
加载类型、模板原文、字段规则、warnings 和 revision。 已有文档草稿或忙碌操作必须先结束；此读取不保存模板。
...
使用与文档相同的解析器和布局生成器展示当前表单内容。 只读预览；使用原始模板的换行格式和 BOM，格式错误保留表单并显示错误。
...
revision 校验 → 保留模板源格式 → 写指纹模板 → 原子更新清单 → 重建。 旧正文/UUID/归属不重写；解析规则改变同类型展示，重建失败给 indexWarning。返回时的索引读取失败会保留窗口供重试。
...
取消新增回到上一个类型；放弃修改恢复最后保存内容；返回关闭窗口。 有未保存内容先确认，已保存配置保留；返回时等待新索引加载完成，期间保持编辑器忙碌。
...
流式上传 → 格式/体积检测 → SHA-256 去重 → UUID 位置 → 登记。 单文件上限 256 MiB；中断清理本次临时文件；取消也终止草稿补丁请求，已成功登记的素材保留在列表。

已有验证入口：[scripts/tests/media.test.mjs](../scripts/tests/media.test.mjs)、[desktop/tests/native-smoke.py](../desktop/tests/native-smoke.py)、[scripts/tests/editor-dialogs.test.mjs](../scripts/tests/editor-dialogs.test.mjs)、[scripts/tests/request-lifecycle.test.mjs](../scripts/tests/request-lifecycle.test.mjs)。
...
文本在光标写 ![] / !video[] / !audio[]；JSON/YAML 走源范围插入。 结构化插入保留缩进、BOM、注释、格式和值；响应返回后重新核对取消状态和草稿，避免覆盖变化；仍需保存正文。
...
只从当前草稿提取媒体；本地 URL 提供图片或原生音视频播放与下载。 不自动播放；音频用 auto 预加载修复 Ogg 时长/跳转；离开/替换草稿预览会暂停播放器。
...
稳定引用写法统一为小写 UUID 的 /asset-files/<UUID>，兼容 assets/ 与 legacyPaths；assetBindings 另进入登记、索引和导出附件清单。 外置素材根由本机绑定解析；当前索引的附件图像补全只取 image，不能把绑定音视频视为正文自动播放器。

已有验证入口：[scripts/tests/workspace-layout.test.mjs](../scripts/tests/workspace-layout.test.mjs)、[scripts/tests/media.test.mjs](../scripts/tests/media.test.mjs)。
...
从已保存原文重新解析，递归携带附属文档、引用素材、元数据和原始 sources；保留附属故事章节层级，按实际文件名区分百分号与空格并匹配旧别名。 不是完整项目恢复包；不把无关素材全部带入；未保存/新建草稿不能导出。 保存或重建忙碌时暂缓打开导出。 包内源路径按每层目录检查大小写/Unicode 及文件目录冲突。
...
正文+稳定登记+项目规则 → 标准文档；文本/JSON/YAML 共用布局。 缓存使用完整源文件名加 .json，避免跨格式重名；旧缓存按原始源路径迁移，原文不重写。

已有验证入口：[scripts/tests/parser.test.mjs](../scripts/tests/parser.test.mjs)、[scripts/tests/metadata-conversion.test.mjs](../scripts/tests/metadata-conversion.test.mjs)、[scripts/tests/build.test.mjs](../scripts/tests/build.test.mjs)。
...
生成 documents/assets/references 三份索引，共用 generation；统一提取图片、视频和音频的嵌套值、旧路径与稳定引用，跳过代码示例，保留裸 ID 声明和明确附件。 未知 ID 和路径保留为未解析引用；resolved 表示找到登记，在线状态见素材索引。原始正文和权威登记不是索引；/api/index 的轻量目录及其内存缓存是另一条链路。

已有验证入口：[scripts/tests/workspace-layout.test.mjs](../scripts/tests/workspace-layout.test.mjs)、[scripts/tests/call-paths.test.mjs](../scripts/tests/call-paths.test.mjs)、[scripts/tests/document-model.test.mjs](../scripts/tests/document-model.test.mjs)。
...
为未登记文档和素材创建稳定记录；保留已有 ID、绑定和归属。 已登记素材变更不会被自动接受为新的权威哈希；构建索引阶段可跳过素材扫描。 扫描也拒绝目录层级的大小写/Unicode 和文件目录冲突。

已有验证入口：[scripts/tests/workspace-layout.test.mjs](../scripts/tests/workspace-layout.test.mjs)、[scripts/tests/generic-project.test.mjs](../scripts/tests/generic-project.test.mjs)。
...
把当前机器的 main 素材根写入 .viento/local.json，运行时解析统一跟随。 这是命令行入口；不把机器绝对路径写进作品清单或迁移包。
...
检查登记唯一性、文件存在、素材指纹、文档关系和模板问题。 输出检查结果；不自动更改原文来迎合规则。

已有验证入口：[scripts/tests/workspace-layout.test.mjs](../scripts/tests/workspace-layout.test.mjs)、[scripts/tests/project-engine.test.mjs](../scripts/tests/project-engine.test.mjs)。
...
| <a id="node-editor"></a>编辑器状态与导航 `editor` | 浏览、搜索、选择、读写、新建和未保存草稿的主协调器。 | [initApp](../web/modules/app-runtime.js#L4967)<br>[web/modules/app-state.js](../web/modules/app-state.js#L1) |
| <a id="node-draft"></a>源码与区块草稿 `draft` | 从当前源码切分区块并按修改片段还原；保留 BOM、换行和未修改文本。 | [createBlockDraft](../web/modules/app-editor-draft.js#L29)<br>[serializeSourceDraft](../web/modules/app-editor-draft.js#L4)<br>[setEditInputMode](../web/modules/app-runtime.js#L2539) |
...
| <a id="node-media_ui"></a>素材选择与草稿预览 `media_ui` | 筛选、导入、复用、粘贴、拖入图片/视频/音频；维护异步草稿预览。 | [setupMediaEditor](../web/modules/app-media-editor.js#L6) |
...
| 节点 | 职责 | 代码依据 |
| --- | --- | --- |
| <a id="node-legacy_ui"></a>旧类别卡片与模板回退 `legacy_ui` | 旧作品未提供通用布局或项目模板时保留英雄等专用卡片/模板分支。 | [web/modules/app-type-templates.js](../web/modules/app-type-templates.js#L1)<br>[web/modules/app-render.js](../web/modules/app-render.js#L1)<br>[renderHeroSkillCards](../web/modules/app-runtime.js#L3130) |
...
| <a id="node-legacy_images"></a>旧图片与技能匹配 `legacy_images` | 无稳定登记等兼容分支保留按名字/旧目录归集图像及英雄技能信息。 | [buildAssetImageCatalog](../scripts/lib/image-index.mjs#L74)<br>[collectHeroSkillsFromSections](../scripts/build-static-hero-skills.mjs#L301) |
...
| <a id="node-doc_service"></a>文档服务 `doc_service` | 读原文、版本校验、写入、文档索引缓存、重建互斥，以及其他业务服务的入口。 | [createDocumentService](../scripts/lib/doc-api-service.mjs#L54) |
| <a id="node-file_store"></a>文档文件事务 `file_store` | 同文档串行读写，快照与原子替换，独占创建；不把展示数据反向写成正文。 | [withDocumentTransaction](../scripts/lib/doc-file-store.mjs#L8)<br>[writeDocumentAtomically](../scripts/lib/doc-file-store.mjs#L36) |
...
| <a id="node-project_service"></a>项目配置与模板服务 `project_service` | 以清单和模板内容生成 revision，先写模板再切换清单，防止过时覆盖。 | [readProjectConfiguration](../scripts/lib/project-service.mjs#L42)<br>[previewProjectTemplate](../scripts/lib/project-service.mjs#L34)<br>[saveProjectTemplate](../scripts/lib/project-service.mjs#L73) |
| <a id="node-media_service"></a>素材导入与清单 `media_service` | 扩展名/内容特征与体积校验、流式上传、哈希去重、稳定 ID 登记。 | [importMediaAsset](../scripts/lib/media-assets.mjs#L53)<br>[listMediaAssets](../scripts/lib/media-assets.mjs#L38) |
| <a id="node-media_insert"></a>结构化素材插入与预览 `media_insert` | JSON/YAML 用源范围补丁插入；文本媒体在光标插入；预览从当前草稿解析媒体。 | [insertStructuredMedia](../scripts/lib/media-insertion.mjs#L12)<br>[prepareMediaInsertion](../scripts/lib/media-insertion.mjs#L76) |
...
| <a id="node-export_pack"></a>Node 导出规划与 ZIP `export_pack` | 分享文档及引用素材，或整库原样打包；校验每层路径的跨系统冲突、登记指纹与源快照。 | [planExport](../scripts/lib/export-package.mjs#L33)<br>[writeExportZip](../scripts/lib/export-package.mjs#L175) |
| <a id="node-rebuild"></a>标准化与索引重建编排 `rebuild` | 先运行标准化子脚本，再构建文档/素材/引用索引；支持指定源范围。 | [rebuildIndex](../scripts/lib/rebuild-workflow.mjs#L70) |
| <a id="node-static"></a>受限文件与素材服务 `static` | 允许列表、路径映射和链接边界；支持 GET/HEAD 与范围响应。 | [handleStaticRequest](../scripts/lib/doc-server-static-routes.mjs#L174)<br>[sendFile](../scripts/lib/doc-server.mjs#L521)<br>[resolveProjectFilePath](../scripts/lib/doc-server.mjs#L634) |
| <a id="node-editable_index"></a>轻量文件目录索引 `editable_index` | /api/index 独立扫描源文件与登记，仅含名称、类型、时间和素材关联；不生成 layout 或归属树。 | [buildEditableDocIndex](../scripts/lib/doc-server.mjs#L344)<br>[createDocumentService](../scripts/lib/doc-api-service.mjs#L54) |
...
| 节点 | 职责 | 代码依据 |
| --- | --- | --- |
| <a id="node-export_render"></a>离线阅读格式 `export_render` | 共享解析布局生成 HTML 或 Markdown，保留附属故事章节层级；素材改为包内路径，保留原文副本。 | [renderExport](../scripts/lib/export-render.mjs#L9) |
...
| <a id="node-registry"></a>文档与素材登记 `registry` | 读取身份登记，增量添加新对象，维护内容指纹、唯一性、注册锁和素材索引。 | [registerWorkspace](../scripts/lib/workspace.mjs#L250)<br>[readRegistry](../scripts/lib/workspace.mjs#L157)<br>[resolveAssetRequest](../scripts/lib/workspace.mjs#L61)<br>[assertPortableFileTree](../scripts/lib/workspace.mjs#L114) |
...
| <a id="node-standardize"></a>原文标准化 `standardize` | 原文 → 项目定义 → 通用解析 → 标准对象；缓存保留完整源文件名，按准确源范围更新和清理。 | [scripts/standardize-docs.mjs](../scripts/standardize-docs.mjs#L1)<br>[buildStandardCatalog](../scripts/standardize-docs/catalog.mjs#L15)<br>[scripts/standardize-docs/sources.mjs](../scripts/standardize-docs/sources.mjs#L1) |
| <a id="node-parser"></a>共享文档解析器 `parser` | 按扩展名解析文本/Markdown、JSON、YAML，保留字段类型、正文、代码、表格及错误原文。 | [parseSourceContent](../scripts/standardize-docs/doc-factory.mjs#L9)<br>[scripts/standardize-docs/parser.mjs](../scripts/standardize-docs/parser.mjs#L1) |
...
| <a id="node-index"></a>文档/素材/引用索引 `index` | 从标准缓存、解析后的素材引用和登记生成三份索引，附加归属导航与未解析引用，共用 generation。 | [scripts/build-static-doc-site.mjs](../scripts/build-static-doc-site.mjs#L1)<br>[buildStandardIndex](../scripts/lib/static-index.mjs#L526)<br>[buildRegistryIndexes](../scripts/lib/workspace.mjs#L404) |
...
| <a id="node-cache"></a>标准缓存与索引 `cache` | 可重建的标准文档、文档索引、素材索引与引用索引。 | [scripts/lib/paths.mjs](../scripts/lib/paths.mjs#L1) |
...
| <a id="node-asset_binding"></a>本机外置素材绑定 `asset_binding` | 仅当前机器的 main 素材根绝对路径；包内恢复为相对 assets/。 | [resolveAssetRoot](../scripts/lib/workspace.mjs#L48) |
...
会话锁位于作品 `.viento/session.lock`；上传暂存位于 `.viento/uploads/`；导出暂存位于 `.viento/cache/exports/`。它们都不是权威创作内容。标准化缓存按完整源文件名追加 `.json`，旧命名随重建按源范围迁移。标准索引与登记必须区分：删除索引可以重建，删除稳定 ID 元数据会丢失身份和关系。
...
| [scripts/lib/doc-api-contract.mjs](../scripts/lib/doc-api-contract.mjs) | 13 |
...
| [scripts/lib/contained-path.mjs](../scripts/lib/contained-path.mjs) | 9 |
...
修订 12 延续 141 份源码指纹、134 处节点源码引用及导入记录，同步 b.2.8.2 版本文件；交互页内嵌数据与 JSON 保持一致。修订 11 已通过启用原生归档互通的 193 项应用回归和 22 项 Rust 测试，原有 1 项真实大体积作品测试忽略；本次升版后的验证见 [发布记录](RELEASE_b.2.8.2.md)。升版前的 [编辑器导出实测](NATIVE_EXPORT_TEST_b.2.8.1.md) 补齐保存等待期间清理缓存、过期后生成、原生覆盖取消与替换，之前的编辑器和作品库记录继续保留，原生报告版本和指纹不改写。功能图仍是静态梳理；内置浏览器此前拒绝本地 HTML 导航，图页面的浏览器交互实测仍未完成。

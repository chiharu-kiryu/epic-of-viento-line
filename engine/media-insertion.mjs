...
// The host validates asset registration and supplies stable asset:<id> references.
export function prepareMediaDraft(content, sourcePath, assets = [], descriptor = {}) {
  const extension = sourceExtension(sourcePath).toLowerCase();
  let updated = content;
  if (assets.length) {
    if (!['.json', '.yaml', '.yml'].includes(extension)) {
      throw invalid('此操作仅用于 JSON 或 YAML；正文素材应在光标处插入。');
    }

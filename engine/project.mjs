...
export function createProjectModel(defaults) {
  function workspacePaths(manifest) {
    return manifest?.version === 3 ? { ...defaults.paths } : { ...LEGACY_PATHS };
  }

  function projectDefinition(manifest) {
    const paths = workspacePaths(manifest);
    const types = manifest?.documentTypes || defaults.documentTypes;
  ...
  function resolveDocumentDefinition(manifest, sourcePath, record = {}) {
  ...
  function projectDocumentDefaults(manifest, sourcePath) {
    const relative = sourcePath.replace(/^docs-standard\//, '').slice(workspacePaths(manifest).documents.length + 1);
    const types = [...(manifest?.documentTypes || defaults.documentTypes)].sort((a, b) => b.directory.length - a.directory.length);
  ...
  return { workspacePaths, projectDefinition, resolveDocumentDefinition, projectDocumentDefaults };
}

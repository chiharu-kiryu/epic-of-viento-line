...
export function renderExport(documents, format, resolveMedia) {
  ...
  function media(value, asset = resolveMedia(value.src)) {
    ...
    if (!html) return value.type === 'image'
      ? `![${escape(caption)}](<${url}>)`
      : `[${value.type === 'audio' ? '音频' : '视频'}：${escape(caption)}](<${url}>)`;
    if (value.type === 'audio') return `<figure><audio controls preload="auto" src="${escapeHtml(url)}" aria-label="${escapeHtml(caption)}"></audio><a href="${escapeHtml(url)}" download>保存原音频</a><figcaption>${escapeHtml(caption)}</figcaption></figure>`;
  ...
  const contents = documents.map((doc, index) => {
    ...
    const sections = doc.layout.sections.map((section) => [
      heading(section.title, level + Math.max(1, (section.level || 2) - 1)),

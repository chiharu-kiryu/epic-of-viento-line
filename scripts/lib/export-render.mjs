...
export function renderExport(documents, format, resolveMedia) {
  ...
  function media(value) {
    ...
    if (!html) return value.type === 'video'
      ? `[视频：${escape(caption)}](<${url}>)`
      : `![${escape(caption)}](<${url}>)`;
    const content = value.type === 'video'
      ? `<video controls preload="metadata" playsinline src="${escapeHtml(url)}" aria-label="${escapeHtml(caption)}"></video><a href="${escapeHtml(url)}" download>保存原视频</a>`
  ...
  const contents = documents.map((doc, index) => {
    ...
    const sections = doc.layout.sections.map((section) => [
      heading(section.title, Math.max(level + 1, section.level || 2)),
      ...(section.blocks || []).map(block),
    ].join('\n\n')).join('\n\n');
    const linked = (doc.linkedAssets || []).filter((asset) => !embedded.has(asset.path)).map((asset) => ['image', 'video'].includes(asset.kind)
  ...
  const toc = documents.length > 1 ? `<nav aria-label="文档目录"><ol>${documents.map((doc, index) => `<li><a href="#document-${index + 1}">${escapeHtml(doc.title)}</a></li>`).join('')}</ol></nav>` : '';
  return `<!doctype html>

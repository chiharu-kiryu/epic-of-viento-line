...
export async function planExport(root, options = {}, signal) {
  ...
  async function validateSnapshot() {
    ...
    for (const entry of snapshots.values()) {
      ...
      if (file !== entry.absolute || signature(await fs.stat(file)) !== signature(entry.stat)) throw exportError(`导出期间文件发生变化，请保存后重试：${entry.relative}`, 409);
    }
    for (const { base, files } of folderSnapshots) {
      if (JSON.stringify(await walkFiles(base)) !== JSON.stringify(files)) throw exportError('导出期间项目文件发生变化，请保存后重试', 409);
    }
    if (JSON.stringify(readWorkspace(root)) !== JSON.stringify(manifest)
      || resolveAssetRoot(root) !== assetRoot
      || JSON.stringify(await readRegistry(root)) !== JSON.stringify(registry)) throw exportError('导出期间项目配置或登记信息发生变化，请重试', 409);
  }
  ...
  if (options.kind === 'workspace') {
    ...
    if (await existing(path.join(root, 'workspace.json'))) await addFile(root, 'workspace.json');
    else addBuffer('workspace.json', json(workspace));
    if (workspace.version >= 2 && await existing(path.join(root, '.viento/workspace.json'))) await addFile(root, '.viento/workspace.json');
...
export async function writeExportZip(plan, output, signal) {
  ...
  const hashes = [];
  let activeInput;
  const fail = (error) => { activeInput?.destroy(error); zip.outputStream.destroy(error); };
  zip.on('error', fail);
    ...
    for (const entry of plan.entries) {
      ...
      zip.addReadStreamLazy(entry.path, { size: entry.stat.size, mtime: entry.stat.mtime, mode: 0o100644,
        ...
        (async () => {
          ...
          activeInput = input.createReadStream();
          void pipeline(activeInput, hashed).catch(fail);
          callback(null, hashed);
        })().catch(callback);
      });
    }
    zip.addReadStreamLazy('manifest.json', (callback) => {
      plan.validateSnapshot().then(() => callback(null, Readable.from(json({ ...plan.archiveManifest, files: hashes }))), callback);
    });
    zip.end();
    await writing;
  } catch (error) { fail(error); await writing.catch(() => {}); throw error; }

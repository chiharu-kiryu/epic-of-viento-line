...
export async function applyProjectDefinition(root, definition, { write = false } = {}) {
  return withRegistryLock(root, async () => {
    ...
    for (const type of definition.documentTypes) {
      ...
      previewProjectTemplate(after, { type, content: await fs.readFile(file, 'utf8'), format: path.extname(file).slice(1).toLowerCase() || 'txt' });
    }
    const changed = JSON.stringify(before) !== JSON.stringify(after);
    let journal = null;
    if (write && changed) {
      journal = `.viento/migrations/project-definition-${Date.now()}.json`;

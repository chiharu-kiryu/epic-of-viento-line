...
import { isMediaValue, splitMediaText } from '../lib/media-format.mjs';
...
function parseTextContent(rawText, relPath = '', options = {}) {
  ...
  const title = firstHeading?.title || (plainTitle ? first.trim() : trimName(path.basename(relPath))) || '未命名文档';
  const blocks = [];
  const sections = [];
  const fields = {};
  const outline = [];
...
function parseStructuredData(data, rawText, relPath, format) {
  const title = trimName(path.basename(relPath)) || '未命名文档';
